...
SetOfRegions::SetOfRegions(const vector<SphericalPoint> &geodesic_in)
      : geodesic(geodesic_in)
{
}

SetOfRegions SetOfRegions::create_from_crossing_counts(
      const std::vector<unsigned> &crossings, const Point &p1,
      const Point &p2)
{
   vector<SphericalPoint> geodesic;
   assert(p1.has_lon_lat && p2.has_lon_lat);
   geodesic.push_back(SphericalPoint(p1));
   geodesic.push_back(SphericalPoint(p2));
   double lon1 = (p1.lon >= 0 ? p1.lon - 180 : p1.lon + 180);
   double lon2 = (p2.lon >= 0 ? p2.lon - 180 : p2.lon + 180);
   geodesic.push_back(SphericalPoint(lon1, -p1.lat));
   geodesic.push_back(SphericalPoint(lon2, -p2.lat));

   SetOfRegions result(geodesic);
...
vector<unsigned> get_horizontal_line_crossing_cnts(
      double y, const std::vector<RegionData> &regs)
{
...
public:
   ...
   PointAngle(const vector<RegionData> &regs, const Point &center_in,
         ...
         : center(&center_in), reg_id(reg_id_in), polygon_id(polygon_id_in),
               pt_id(pt_id_in)
   {
   ...
   bool operator<(const PointAngle &other) const
   {
...
double PointAngle::normalize_angle(double alpha)
{
   while (alpha < -M_PI)
      alpha += 2 * M_PI;
   while (alpha >= M_PI)
...
void PointAngle::fill_alt_angle()
{
   alt_angle = angle;
   if (alt_angle < 0)
      alt_angle += M_PI;
...
void find_lines_from_center(MaxSetList *set_list, const Point &center,
...
void find_lines(MaxSetList *set_list, const vector<RegionData> &regs)
{
   for (const RegionData &reg : regs)
   {
